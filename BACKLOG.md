...
- ~~**Extract an `_AihwSa4Dataset` base class.**~~ **Done** — the five
  near-identical AIHW SA4→SA2 fetchers now subclass
  `_aihw_sa4_base.AihwSa4Dataset`, declaring only their per-dataset
  schema specifics (URL registry, encoding, member-match substrings,
  filter dimension+value(s), SA4 code format, measure→column map) as
  class attributes. Pure refactor, zero behaviour change (827 tests
  unchanged). The next AIHW dataset is now a ~40-line config subclass.
