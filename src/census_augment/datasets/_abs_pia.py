...
class AbsPiaDataSource(_AbsXlsxDataset):
    ...
    :class:`_AbsXlsxDataset` base. ``release`` is the reference
    financial year (e.g. ``"2022-23"`` for the FY 2022-23 release).
    ...
    def _post_parse(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # which release the values come from.
...
def _build_fetcher(root: Path, release: str | None = None) -> AbsPiaDataSource:
    kwargs: dict[str, object] = {"root": root}
    if release is not None:
        kwargs["release"] = release
    return AbsPiaDataSource(**kwargs)  # type: ignore[arg-type]
